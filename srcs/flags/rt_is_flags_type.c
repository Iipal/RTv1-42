...
/*   Updated: 2019/12/10 17:45:41 by tmaluh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	rt_is_flag_boolean(char const *flag)
{
	...
	out = 0UL;
	if ('-' == *flag && '-' != *(flag + 1))
		out = ft_is_one_of_n(flag, MAX_BOOLEAN_FLAGS,
...
size_t	rt_is_flag_wparam(char const *flag)
{
	...
	out = 0UL;
	if ('-' == *flag && '-' != *(flag + 1))
		out = ft_is_one_of_n(flag, MAX_WPARAMS_FLAGS,
...
size_t	rt_is_flag_other(char const *flag)
{
	...
	out = 0UL;
	if ('-' == *flag && '-' != *(flag + 1))
		out = ft_is_one_of_n(flag, MAX_OTHER_FLAGS, SF_NB);
	else if ('-' == *flag && '-' == *(flag + 1))
		out = ft_is_one_of_n(flag, MAX_OTHER_FLAGS, F_NB);
	return (out);
}
