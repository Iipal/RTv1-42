...
/*   Updated: 2019/06/05 23:51:31 by tmaluh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rt_rendering(Environment *restrict const env)
{
	...
	while (RT_CANVAS_ENDY > ++Y(i) && (X(i) = RT_CANVAS_STARTX))
		while (RT_CANVAS_ENDX > ++X(i))
		{
			env->tmax = TMAX;
			env->tmin = TMIN;
			clr = rt_raytracing(env, (Vector){X(i) * WIN_X / (1000.0 * WIN_X),
	...
	if (env->flags.random_lights_intense && !env->flags.no_calc_light)
		rt_randomatic_lights_intense(env->scene.lights, env->scene.ins_lights);
